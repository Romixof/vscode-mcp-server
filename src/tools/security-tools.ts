...
WHEN TO USE: to review what connected clients did on this machine, or to investigate suspicious activity.`, {
                ...
                kind: z.enum(['tool_call', 'tool_denied', 'shell_blocked', 'sandbox_violation', 'consent_granted', 'consent_denied', 'token_revoked', 'key_created', 'key_revoked']).optional().describe('Filter by event kind')
        }, async ({ limit = 50, kind }) => {
