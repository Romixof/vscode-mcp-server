...
export function registerWorkflowTools(server: McpServer, terminal?: vscode.Terminal): void {
        ...
        When the same name exists in several sources, qualify it with its source prefix (npm:, composer:, make:), e.g. "make:build".`,
                ...
                async ({ task = '', args = '', cwd = '.', timeout = 120000, workspace }): Promise<CallToolResult> => {
                        if (task && task.trim()) {
                                const pm = planModeIntercept('run_task_code', `{ task: "${task}"${args ? `, args: ${JSON.stringify(args)}` : ''} }`);
                                if (pm) { return pm; }
                        }
        ...
        Detection order: package.json "build" script, "compile" script, Makefile, tsconfig.json. Pass an explicit command to override detection.`,
                ...
                async ({ command = '', cwd = '.', timeout = 300000, workspace }): Promise<CallToolResult> => {
                        const pm = planModeIntercept('build_project_code', `{ ${command.trim() ? `command: ${JSON.stringify(command.trim())}` : 'command: (auto-detected)'} }`);
                        if (pm) { return pm; }
                        const dir = resolveWorkingDir(getWorkspaceRoot(workspace), cwd);
        ...
        File format: { "dev": { "command": "npm run dev --host", "description": "Start dev server" }, "lint": "eslint ." }. Values can be a plain command string or an object with command and description. Call without a name to list aliases.`,
                ...
                async ({ name = '', args = '', cwd = '.', timeout = 120000, workspace }): Promise<CallToolResult> => {
                        if (name.trim()) {
                                const pm = planModeIntercept('run_alias_code', `{ name: "${name}"${args ? `, args: ${JSON.stringify(args)}` : ''} }`);
                                if (pm) { return pm; }
                        }
                        const root = getWorkspaceRoot(workspace);
