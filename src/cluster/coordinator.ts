...
export class ClusterCoordinator {
        ...
        async joinAfterAddressInUse(originalError: Error): Promise<void> {
                ...
                try {
                        const response = await fetchWithTimeout(`http://127.0.0.1:${this.preferredPort}${CLUSTER_IDENTITY_PATH}`, { headers: { ...this.authHeaders() } }, IDENTITY_TIMEOUT_MS);
                        identity = response.ok ? await response.json().catch(() => undefined) : undefined;
                } catch {
                        identity = undefined;
                }
        ...
        private async becomeSpoke(hubPort: number): Promise<void> {
                ...
                for (let attempt = 0; attempt < 6; attempt++) {
                        try {
                                ...
                                lastError = error;
                                logger.info(`[cluster] Register attempt ${attempt + 1}/6 failed: ${error instanceof Error ? error.message : String(error)}`);
        ...
        private async registerOnce(): Promise<void> {
                ...
                const body = await response.json().catch(() => undefined) as { ok?: boolean; assignedName?: string; labelOverrides?: Record<string, string>; detail?: string; error?: string } | undefined;
                if (!response.ok || !body?.ok) {
                        const detail = body?.detail ?? body?.error ?? `HTTP ${response.status}`;
                        throw new Error(`registration refused (${detail})`);
                }
        ...
        private async runElection(): Promise<void> {
                ...
                try {
                        ...
                        try {
                                const response = await fetchWithTimeout(`http://127.0.0.1:${this.preferredPort}${CLUSTER_IDENTITY_PATH}`, { headers: { ...this.authHeaders() } }, IDENTITY_TIMEOUT_MS);
                                const identity = response.ok ? await response.json().catch(() => undefined) as { role?: string; protocol?: number; extensionVersion?: string } | undefined : undefined;
