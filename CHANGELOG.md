...
- Starting the server on a port that another VS Code window already holds no longer leaves the second window half-started and silent: startup now fails with an explicit message naming the port and the likely cause, the status bar shows the server off, and toggling again retries once the port frees up.
